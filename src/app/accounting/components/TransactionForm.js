...
export default function TransactionForm({ account, onClose }) {
  ...
  // Use local time for datetime-local default (toISOString is UTC and shifts time)
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Refresh immediately and close dialog (no wait)
        router.refresh();
        onClose();
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg border border-gray-200 flex flex-col max-h-[90vh]">
        ...
        {!success ? (
          ...
          >
            ...
            {(type === "credit" || (isTransfer && (type === "debit" || type === "both"))) && (
              <div className="space-y-1.5 animate-in zoom-in-95 duration-200">
                <label className="text-xs font-bold text-gray-700">
                  Destination {isTransfer && (type === "debit" || type === "both") ? "*" : "(Optional)"}
                </label>
                <input
                  type="text"
                  name="destination"
                  required={isTransfer && (type === "debit" || type === "both")}
