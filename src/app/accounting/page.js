...
import { requireAccountingAccess } from "@/app/lib/auth/getSession";
...
export default async function AccountingPage({ searchParams }) {
  try {
    // Check if user has accounting access - redirect if not
    await requireAccountingAccess();
  } catch (error) {
    // Log the error for debugging
    console.error("[AccountingPage] Access denied:", error.message);
    // If not accounting role, redirect to dashboard
    redirect("/dashboard");
  }
