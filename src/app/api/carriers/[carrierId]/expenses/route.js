...
export async function POST(request, { params }) {
  ...
  try {
    ...
    // Validate category - service (stored as 'maintenance') and tyre are not allowed for trips (only for trucks)
    const validCategories = ["fuel", "driver_rent", "taxes", "tool_taxes", "on_road", "others"];
    if (!validCategories.includes(category)) {
      return NextResponse.json(
        { error: "Invalid category. Service and tyre expenses are for trucks only, not trips." },
        { status: 400 }
      );
    }
    
    // Explicitly reject service and tyre categories
    if (category === "maintenance") {
      return NextResponse.json(
        { error: "Service expenses are for trucks only, not trips. Please add service expenses from the truck detail page." },
        { status: 400 }
      );
    }
