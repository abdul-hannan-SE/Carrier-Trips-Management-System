...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    // Validate category if provided - fuel is trip-only
    if (category) {
    ...
    // Fuel is trip-only; truck fuel entries are synced and should not be edited here
    let finalAmount = amount ? parseFloat(amount) : existingExpense.amount;
    if (existingExpense.category === "fuel") {
      return NextResponse.json(
        { error: "Fuel expenses cannot be edited from truck. Please edit fuel from the trip." },
        { status: 400 },
      );
    }
