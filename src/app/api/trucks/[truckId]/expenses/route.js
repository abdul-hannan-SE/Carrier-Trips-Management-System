...
export async function GET(request, { params }) {
  ...
  try {
    ...
    // Build query
    // Fuel is trip-only; any truck fuel entries are synced from trip fuel (syncedFromExpense).
    // We exclude those here to avoid duplicates and to enforce "fuel from trip only".
    const query = {
      truck: truckObjectId,
      $or: [
        { category: { $ne: "fuel" } },
    ...
    // Filter by category (fuel is trip-only)
    if (category && ["maintenance", "tyre", "others"].includes(category)) {
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    // Validate category - fuel is trip-only (it gets synced to truck automatically)
    const validCategories = ["maintenance", "tyre", "others"];
    if (!validCategories.includes(category)) {
      return NextResponse.json(
        {
          error:
            "Invalid category. Only maintenance, tyre, and others are allowed for trucks. Fuel must be added from trips.",
        },
        { status: 400 },
      );
    }
    ...
    // If this is a service expense (stored as category 'maintenance'), update truck's last service and current meter reading
