...
export async function getAllCarriers(searchParams = {}) {
  ...
  try {
    ...
    // Filter by specific truck (central dashboard / truck detail views)
    if (searchParams.truckId) {
      const truckIdRaw = decodeURIComponent(String(searchParams.truckId)).trim();
      if (truckIdRaw && mongoose.Types.ObjectId.isValid(truckIdRaw)) {
        carrierMatchConditions.push({
          truck: new mongoose.Types.ObjectId(truckIdRaw),
        });
      } else if (truckIdRaw) {
        // If provided but invalid, force empty results
        carrierMatchConditions.push({
          truck: new mongoose.Types.ObjectId("000000000000000000000000"),
        });
      }
    }
  ...
  try {
    ...
    try {
      ...
      if (truck && tripDistance > 0) {
        ...
        // Check if service is needed and store warning if exceeded
        ...
        if (kmsRemaining <= 0) {
          // Service overdue - store this info
          await Truck.findByIdAndUpdate(truck._id, {
            $set: {
              maintenanceWarning: `Service overdue! Current: ${newKm}km, Next required: ${nextMaintenanceKm}km`,
            },
          });
          console.warn(
            `Truck ${truck.name} service is overdue! Current: ${newKm}km, Last service: ${updatedTruck.lastMaintenanceKm}km`,
          );
        } else if (kmsRemaining <= 500) {
          // Service due soon
          await Truck.findByIdAndUpdate(truck._id, {
            $set: {
              maintenanceWarning: `Service due soon! ${kmsRemaining}km remaining until ${nextMaintenanceKm}km`,
            },
          });
          console.warn(
            `Truck ${truck.name} service due soon! ${kmsRemaining}km remaining`,
          );
        } else {
  ...
  try {
    ...
    if (carrier.truck && carrier.distance) {
      ...
      if (truck) {
        ...
        // Update service warning based on new meter reading
        const nextMaintenanceKm =
        ...
        if (kmsRemaining <= 0) {
          // Service overdue
          truck.maintenanceWarning = `Service overdue! Current: ${newMeterReading}km, Next required: ${nextMaintenanceKm}km`;
        } else if (kmsRemaining <= 500) {
          // Service due soon
          truck.maintenanceWarning = `Service due soon! ${kmsRemaining}km remaining until ${nextMaintenanceKm}km`;
        } else {
          // Clear warning if all good
